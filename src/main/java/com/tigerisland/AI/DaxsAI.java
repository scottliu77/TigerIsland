...
public class DaxsAI extends AI {
    ...
    private ArrayList<Location> serverSafeValidTotoroPlacements;
    ...
    private void gatherBuildActionInfo() {
        ...
        this.serverSafeValidTotoroPlacements = getServerSafeTotoroLocations();
    ...
    private ArrayList<Location> getServerSafeTotoroLocations() {
        ArrayList<Location> serverSafeLocations = new ArrayList<Location>();
        for(Location location : validTotoroPlacements) {
            if(turnState.getBoard().findAdjacentSettlementsToLocation(location).size() == 1) {
                serverSafeLocations.add(location);
            }
        }
        return serverSafeLocations;
    }

    private boolean canPlaceTotoro(){
        return this.serverSafeValidTotoroPlacements.size()>0 && hasATotoro();
    }
    ...
    private void placeTotoro(){
        ...
        int randInt = rand.nextInt(serverSafeValidTotoroPlacements.size());
        buildLocation = serverSafeValidTotoroPlacements.get(randInt);
        if(tilePlacement == null) {
            tilePlacement = validTilePlacements.get(0);
        }
    }
